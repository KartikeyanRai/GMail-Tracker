...
function withFreshToken(interactive = false) {
  return new Promise(resolve => {
    chrome.identity.getAuthToken({ interactive }, token => {
      if (chrome.runtime.lastError || !token) {
        console.warn("[Gmail Tracker] Token fetch failed:", chrome.runtime.lastError?.message);
        return resolve(null);
      }
      state.accessToken = token;
...
async function syncTrackedEmails() {
  ...
  if (!token) return console.warn("[Gmail Tracker] ❌ No valid token—sync aborted");
  ...
  try {
    ...
    console.log("[Gmail Tracker] Backend response:", raw);
    ...
    const trackedMap = {};
    emails.forEach(e => {
      trackedMap[e.messageId] = { isRead: e.readCount > 0, readCount: e.readCount };
    });

    chrome.storage.local.set({ trackedEmails: trackedMap }, () => {
      console.log("✅ Local storage updated:", Object.keys(trackedMap).length);
    });
...
//   // Remove cached token if it might be expired
//   if (state.accessToken) {
//     chrome.identity.removeCachedAuthToken({ token: state.accessToken }, () => {
//       chrome.identity.getAuthToken({ interactive: false }, (newToken) => {
//         if (chrome.runtime.lastError || !newToken) {
//           console.warn("[Gmail Tracker] ❌ Failed to refresh token:", chrome.runtime.lastError?.message);
//           return;
//         }

//         console.log("[Gmail Tracker] 🔁 Refreshed token:", newToken);
//         state.accessToken = newToken;
//         chrome.storage.local.set({ accessToken: newToken });

//         syncTrackedEmails(); // Proceed after getting a fresh token
//       });
//     });
//   } else {
//     chrome.identity.getAuthToken({ interactive: false }, (token) => {
//       if (chrome.runtime.lastError || !token) {
//         console.warn("[Gmail Tracker] ❌ Failed to get token:", chrome.runtime.lastError?.message);
//         return;
//       }

//       console.log("[Gmail Tracker] ✅ Retrieved token:", token);
//       state.accessToken = token;
//       chrome.storage.local.set({ accessToken: token });

//       syncTrackedEmails(); // Proceed after getting a token
//     });
//   }
// }
...
//     });
//     const raw = await res.text();
//     console.log("[Gmail Tracker] Using accessToken:", state.accessToken);
//     if (!res.ok) {
//       console.warn("[Gmail Tracker] ❌ Backend returned error:", raw);
//       throw new Error(raw);
//     }
//     const json = JSON.parse(raw);
//     console.log("[Gmail Tracker] 📥 Raw response from /tracked:", json);
